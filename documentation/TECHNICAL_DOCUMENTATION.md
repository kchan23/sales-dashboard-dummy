...
The default demo parquet files are generated by `scripts/generate_demo_data.py`. Customer records are synthetic and PII-free; repeat identifiable customers are modeled by assigning the same synthetic `customer_id` to orders on clustered business dates using a presentation-scaled visit-day target.
