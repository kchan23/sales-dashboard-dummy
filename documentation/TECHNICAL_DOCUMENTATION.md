...
Demo customer records reuse synthetic `customer_id` values across dates so repeat-visit analytics render realistically in presentation mode.
...
The default demo parquet files are generated by `scripts/generate_demo_data.py`. Customer records are synthetic and PII-free; repeat identifiable customers are modeled by assigning the same synthetic `customer_id` to orders on multiple business dates using the live Customer Analytics visit-day distribution as a scaled target shape.
